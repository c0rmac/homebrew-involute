class InvoluteMlx < Formula
  desc "Hardware-accelerated CBO library for manifold-constrained optimization — Apple MLX backend"
  ...
  depends_on "libomp"
  depends_on "mlx"
  depends_on "c0rmac/homebrew-isomorphism/isomorphism-mlx"
  depends_on "c0rmac/homebrew-riemannian-gaussian-sampler/riemannian-gaussian-sampler-mlx"
  ...
  def install
    ...
    args = std_cmake_args + [
      ...
      "-DBUILD_TESTING=OFF",
      "-DUSE_MLX=ON",
      "-DCMAKE_PREFIX_PATH=#{iso_prefix};#{sampler_prefix};#{mlx_prefix};#{HOMEBREW_PREFIX}",
    ]
